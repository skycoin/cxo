...
func (msg *AnnounceMessage) HandleFromUpstream(
	cc *nodeManager.UpstreamContext) error {
	//fmt.Println("AnnounceMessage")

	if DB.Has(msg.Hash) {
		return fmt.Errorf("received an announce for data I already have: %v",
			msg.Hash)
	}
...
func (msg *RequestMessage) HandleFromDownstream(
	cc *nodeManager.DownstreamContext) error {

	// fetch from DB the data by hash
	responseData, ok := DB.Get(msg.Hash)
	if !ok {
		return fmt.Errorf("received request for data I don't have: %v",
			msg.Hash)
	}
...
func (msg *DataMessage) HandleFromUpstream(
	cc *nodeManager.UpstreamContext) error {
