...
func commandDispatcher(c *RPCClient) bool {
	cmd, args := cliInput("\n> ")
	fmt.Print("\n")
...
func caseAdd(c *RPCClient, args []string) {
...
func caseRemove(c *RPCClient, args []string) {
	cmd, args := args[0], args[1:]
	switch cmd {
	case _Board:
		removeBoard(c, args)
...
func caseList(c *RPCClient, args []string) {
	cmd, args := args[0], args[1:]
	switch cmd {
	case "boards", _Board:
		listBoards(c, args)
